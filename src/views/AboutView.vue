...
export default {
  ...
  setup() {
    ...
    const load = () => {
      axios
        .get("/api/work/findListByPageUser", {
          params: {
    ...
    const save = () => {
      if (state.form.id) {
        axios.post("/api/work/updateUser", state.form).then((res) => {
          if (res.data.code === 0) {
            ElMessage({
              message: "修改成功",
              type: "success",
        ...
        axios.post("/api/work/addUser", state.form).then((res) => {
          if (res.data.code === 0) {
            ElMessage({
              message: "添加成功",
              type: "success",
    ...
    const handleDelete = (id) => {
      console.log("id" + id);
      axios.get("/api/work/deleteUserById/" + id).then((res) => {
        if (res.data.code === 0) {
          ElMessage({
            message: "删除成功",
            type: "success",
